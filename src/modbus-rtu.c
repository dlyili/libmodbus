...
// add by jjj
static int buffer_ser_read(modbus_rtu_t* ctx_rtu, uint8_t* p_msg,
	unsigned int max_len)
{
#if defined(_WIN32)
	unsigned int n = ctx_rtu->w_ser.n_bytes;
#else
	unsigned int n = ctx_rtu->n_bytes;
#endif
	if (max_len < n) {
		n = max_len;
	}
	...
	{
#if defined(_WIN32)
		memcpy(p_msg, ctx_rtu->w_ser.buf, n);
#else
		memcpy(p_msg, ctx_rtu->buf, n);
#endif
	}
...
static ssize_t _modbus_rtu_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
	...
	if (ctx_rtu->use_buffer)
	{
		return buffer_ser_read(ctx_rtu, rsp, rsp_length);
    }
	else
	{
#if defined(_WIN32)
...
static int _modbus_rtu_check_integrity(modbus_t *ctx, uint8_t *msg, const int msg_length)
{
    ...
    if (slave != ctx->slave && slave != MODBUS_BROADCAST_ADDRESS) {
        ...
        //edit by jjj return message len
        return msg_length;// 0;
    }
...
#else
			memcpy(rtu_ctx->buf, rtu_ctx->read_buf, n);
			rtu_ctx->n_bytes = n;
#endif
		}
...
modbus_t *
modbus_new_rtu(const char *device, int baud, char parity, int data_bit, int stop_bit, int use_buffer)
{
    modbus_t *ctx;
    modbus_rtu_t *ctx_rtu;
...
//////////////////////////////////////////////////////////////////////////
//add by jjj
int modbus_rtu_set_read_buffer(modbus_t* ctx, const void* buffer, uint8_t bytes)
{
...
int modbus_rtu_get_write_buffer(modbus_t* ctx, void* buffer, uint8_t* bytes)
{
	modbus_rtu_t* ctx_rtu = NULL;
	if (ctx == NULL || buffer == NULL || bytes == NULL)
	{
		errno = EINVAL;
		return -1;
	}
	...
	ctx_rtu = (modbus_rtu_t*)ctx->backend_data;
	*bytes = ctx_rtu->n_write_bytes;
	memcpy(buffer, ctx_rtu->write_buf, ctx_rtu->n_write_bytes);
